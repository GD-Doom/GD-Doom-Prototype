...
static void CalcHeight(Player *player)
{
    ...
    if (!cur_sec->height_sector && onground)
...
bool PlayerThink(Player *player)
{
    ...
    if (erraticism.d_)
    {
        bool    sinking = false;
        Sector *cur_sec = player->map_object_->subsector_->sector;
        if (!cur_sec->height_sector && cur_sec->sink_depth > 0 &&
    ...
    if (player->map_object_->region_properties_->special || player->underwater_ || player->swimming_ ||
        player->airless_ || player->map_object_->subsector_->sector->has_deep_water)
    {
