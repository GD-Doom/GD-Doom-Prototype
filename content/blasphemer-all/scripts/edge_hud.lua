...
function blasphemer_status_bar()
...
function blasphemer_overlay_status()
    local RelX

    hud.text_font("DEFAULT_BIG")
    hud.set_scale(1.0)
...
function blasphemer_automap()
    ...
    if (which == 0) then --blasphemer_status_bar()
        ...
        hud.set_scale(1.0)
        blasphemer_status_bar()
        blasphemer_life_gem()
    end
    if (which == 1) then --blasphemer_overlay_status()
...
function begin_level()
    ...
    --	Blasphemer trims your inventory on each new map
...
function draw_all()
    hud.coord_sys(320, 200) --Doesn't Blasphemer draw at 320x240?
    hud.grab_times()
    ...
    -- the first is the usual Blasphemer status bar
    ...
    -- Dasho - Revisit after autoload rework
    --edge_air_bar()
    --edge_time_bar()
    --edge_footsteps()
end
